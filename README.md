...
Permissions on a network, are rights that are given to the users of the network. These rights will be what data a user on a network will have access to. Permissions are important because it allows network administrators to properly control their network, this is because the administrators can decide what user can do on their network. For example a online website would need to give permissions to internet users so they can access their network's web server, however they would to limit permissions to users so they cannot make changes to the web server. The admins will also need to give higher permissions to themselves so they can access data on each employee's workstation and so they can make changes to webpages on the website.
